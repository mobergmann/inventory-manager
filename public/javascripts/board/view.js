...
function init(next = () => {}) {
    ...
    // retrieve project_id by current url
...
//#region GET
    ...
    function display_inventory(inventory, owner) {
        ...
        let remove_btn = clone.querySelectorAll(".inventory-remove-button")[0];
        remove_btn.setAttribute("inventoryid", inventory.id);
    ...
    function load_inventories(next = function () {
        ...
        xhr.open('GET', `/api/inventories/${_project.id}`);
        xhr.setRequestHeader("Content-Type", "application/json");
        xhr.send();
    }
...
//#region Player
    ...
    function load_players(next = function () {
        ...
        xhr.open('GET', `/api/users/${_project.id}`);
        xhr.setRequestHeader("Content-Type", "application/json");
        xhr.send();
    }
            ...
            load_inventories(() => {
                load_items(() => {
                    load_money(() => {
                        ...
                        let elems = document.querySelectorAll(".manage");
                        elems.forEach(elem => {
